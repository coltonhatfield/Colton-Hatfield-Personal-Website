    ...
    <section id="projects" className="pt-24 mt-[-6rem]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-[#333] pt-8">
        {projects.map((project, idx) => (
          ...
          >
            <div>
              <span className="absolute top-4 right-4 font-mono text-[10px] text-[#A3A3A3] tracking-widest">[{project.year}]</span>
              <h3 className="text-xl font-bold mb-4 uppercase text-white group-hover:text-[#F0B800] transition-colors pr-12">
                {project.title}
              </h3>
              <p className="text-sm text-[#A3A3A3] leading-relaxed mb-6 font-sans line-clamp-3">
                {project.description}
              </p>
            </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mt-auto">
              ...
              <div className="flex flex-row gap-2 mt-4 lg:mt-0 lg:ml-auto shrink-0 w-full lg:w-auto">
                <Link to={`/project/${project.id}`} className="flex-1 lg:flex-none flex justify-center items-center gap-2 text-[10px] font-mono tracking-widest uppercase text-black bg-[#F0B800] hover:bg-white transition-colors px-2 py-2" aria-label={`View details for ${project.title}`}>
                  <span>View Details</span>
                  <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4" />
                </Link>
                {project.github && (
                  <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex-1 lg:flex-none flex justify-center items-center gap-2 text-[10px] font-mono tracking-widest uppercase text-[#E0E0E0] border border-[#333] hover:border-[#F0B800] hover:text-[#F0B800] transition-colors px-2 py-2" aria-label={`GitHub Repository for ${project.title}`}>
                    <Github className="w-3 h-3 sm:w-4 sm:h-4" />
                    <span>View on GitHub</span>
                  </a>
                )}
                {project.link && (
                  <a href={project.link} target="_blank" rel="noopener noreferrer" className="flex-1 lg:flex-none flex justify-center items-center gap-2 text-[10px] font-mono tracking-widest uppercase text-[#E0E0E0] border border-[#333] hover:border-[#F0B800] hover:text-[#F0B800] transition-colors px-2 py-2" aria-label={`Live Project for ${project.title}`}>
